...
| `notifier.py` | シグナルをスプレッドシートに書き込み | Google Sheets (`Signals_YYYYMMDD` 形式の日別シート) |
...
シート名は `Signals_YYYYMMDD` 形式（例: `Signals_20260120`）で日別に作成されます。
...
| 設定項目 | 推奨値 | 理由 |
|----------|--------|------|
| **スケジュールされた時刻にタスクを開始できなかった場合、すぐにタスクを実行する** | ✅有効 | PC未起動時にミスした実行を次回起動時にリカバリ |
| **コンピューターをAC電源で使用している場合のみタスクを開始する** | ❌無効 | バッテリー動作時も実行可能に |
| **コンピューターがバッテリ電源に切り替わった場合、停止する** | ❌無効 | 実行中断を防止 |

> [!IMPORTANT]
> これらの設定がないと、PCがログオフ/スリープ中はバッチがスキップされ、データ欠損が発生します。
